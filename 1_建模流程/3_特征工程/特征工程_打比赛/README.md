...
对每个特征衍生特征，衍生方案：<br>
 - 对数值型特征，先进行计数和排序（横向衍生两列特征） 2.1.1
 - 针对同类特征群（比如消费，浏览记录等）横向扩展，计算一些统计量作为特征 2.2.1
 - 然后再衍生\[纵向]聚合特征(类别特征和数值型特征) 2.2.1, 2.2.2
 - 再进行特征减法
 - 最后单特征及单特征衍生特征跑一下模型比如lgb，看下效果（这里是AUC值）
 特征auc低的比如小于0.7的可以去掉

取效果好的前几个特征，然后造多项式特征 + 横向衍生的几个特征(计数和排序、同类特征群、时间特征) #2.1<br>
然后跑纵向扩展加法 #2.2<br>
特征减法 #3<br>
模型效果cv #4<br>
